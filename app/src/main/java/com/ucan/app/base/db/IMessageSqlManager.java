...
public class IMessageSqlManager extends AbstractSQLManager {
	...
	public static final String ACTION_SESSION_DEL = "com.ucan.app.intent.ACTION_SESSION_DEL";
	public static final String ACTION_GROUP_DEL = "com.ucan.app.intent.ACTION_GROUP_DEL";
	...
	public static void checkContact(String contactid, String username) {
		...
		if (!ContactSqlManager.hasContact(contactid)) {
			Contact c = ContactSqlManager.getCacheContact(contactid);
			if (c == null) {
				c = new Contact(contactid);
		...
		try {
			if (!TextUtils.isEmpty(message.getSessionId())) {
				...
				ownThreadId = ConversationSqlManager
						.querySessionIdForBySessionId(contactIds);
				if (ownThreadId == 0) {
					try {
						ownThreadId = ConversationSqlManager
				...
				if (ownThreadId > 0) {
					...
					if (boxType == IMESSENGER_BOX_TYPE_DRAFT) {
						try { // 草稿箱只保存文本
							values.put(IMessageColumn.OWN_THREAD_ID,
									ownThreadId);
							values.put(IMessageColumn.sender, message.getForm());
							values.put(IMessageColumn.MESSAGE_ID,
									message.getMsgId());
							values.put(IMessageColumn.MESSAGE_TYPE, message
									.getType().ordinal());
							values.put(IMessageColumn.SEND_STATUS, message
									.getMsgStatus().ordinal());
							values.put(IMessageColumn.READ_STATUS, isread);
							values.put(IMessageColumn.BOX_TYPE, boxType);
							values.put(IMessageColumn.BODY,
									((ECTextMessageBody) message.getBody())
											.getMessage());
							values.put(IMessageColumn.USER_DATA,
									message.getUserData());
							values.put(IMessageColumn.RECEIVE_DATE,
									System.currentTimeMillis());
							values.put(IMessageColumn.CREATE_DATE,
									message.getMsgTime());
							...
							// ChattingFragment.isFireMsg=false;阅后即焚
							values.clear();
						}
					} else {
						try {
							...
							if (message.getType() == Type.VIDEO
								...
								ECVideoMessageBody videoBody = (ECVideoMessageBody) message
										.getBody();
								values.put(IMessageColumn.BODY,
										videoBody.getLength() + "");
							}
							...
							/*
							 * 阅后即焚 if(message.getType()==Type.IMAGE&&message.
							 * getDirection
							 * ()==Direction.SEND&&ChattingFragment.isFireMsg){
		...
		if (cursor != null && cursor.getCount() > 0) {
			if (cursor.moveToFirst()) {
				...
				String sessionid = cursor.getString(cursor
						.getColumnIndexOrThrow(IThreadColumn.THREAD_ID));
	...
	public static int updateIMessageDownload(ECMessage msg) {
		...
		try {
			...
			if (msg.getType() == ECMessage.Type.VOICE) {
				int voiceTime = VeryUtils.calculateVoiceTime(msgBody
						.getLocalUrl());
				...
				if (detail.getType() != ECMessage.Type.TXT) {
					...
					if (!TextUtils.isEmpty(body.getLocalUrl())) {
						if (body.getLocalUrl().startsWith("THUMBNAIL://")) {
							Image imgInfo = ImgInfoSqlManager.getInstance()
	...
	public static ECMessage packageMessage(Cursor cursor) {
		...
		if (msgType == ECMessage.Type.TXT.ordinal()) {
			...
			if (msgType == ECMessage.Type.VOICE.ordinal()) {
				...
				fileBody.setFileName(VeryUtils
	...
	public static void updateMsgReadStatus(String msgId, boolean isRead) {
		try {
			...
			getInstance().sqliteDB().update(
					DatabaseHelper.TABLES_NAME_IM_MESSAGE, values, "msgid = ?",
					new String[] { msgId });
