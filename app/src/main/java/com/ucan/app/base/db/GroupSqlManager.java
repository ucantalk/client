...
public class GroupSqlManager extends AbstractSQLManager {
	...
	public static Cursor getGroupCursor(boolean isDiscussion) {
		...
		try {
			if (!isDiscussion) {
				...
				sql = "select groupid, name, type, count ,permission ,joined from "
						+ DatabaseHelper.TABLES_NAME_GROUPS_2
						+ " where isdiscussion=1 and joined =1 order by joined desc , create_date desc";
			} else {
				sql = "select groupid, name, type, count ,permission ,joined from "
						+ DatabaseHelper.TABLES_NAME_GROUPS_2
						+ " where isdiscussion=0 and joined =1 order by joined desc , create_date desc";
			}
	...
	public static List<ECGroup> getJoinGroups() {
		...
		try {
			String sql = "select groupid, name from "
					+ DatabaseHelper.TABLES_NAME_GROUPS_2
					+ " where isdiscussion=0 and joined=1 order by create_date desc";
			;
	...
	public static boolean getJoinState(String groupId) {
		String sql = "select groupid, joined from "
				+ DatabaseHelper.TABLES_NAME_GROUPS_2 + " where groupId = '"
				+ groupId + "'";
		Cursor cursor = getInstance().sqliteDB().rawQuery(sql, null);
		if (cursor != null && cursor.getCount() > 0) {
	 ...
	 */
	public static ArrayList<Long> insertGroupInfos(List<ECGroup> imGroups,
			int joined) {
		...
		try {
			synchronized (getInstance().mLock) {
				...
				for (ECGroup imGroup : imGroups) {
					try {
						long row = insertGroup(imGroup, joined == 1,
								joined == -1, false);
						if (row != -1) {
							rows.add(row);
						}
	...
	public static ArrayList<Long> insertDisGroupInfos(List<ECGroup> imGroups,
			int joined) {
	 ...
	 * @param join
	 */
	public static long insertGroup(ECGroup group, boolean join,
			boolean ignoreJoin, boolean isDiscussion) {
		...
		try {
			...
			if (isExitGroup(group.getGroupId())) {
				return getInstance().sqliteDB().update(
						DatabaseHelper.TABLES_NAME_GROUPS_2, values,
						"groupid = ?", new String[] { group.getGroupId() });
			}
			long rowId = getInstance().sqliteDB().insert(
					DatabaseHelper.TABLES_NAME_GROUPS_2, null, values);
			return rowId;
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
	...
	public static boolean isDiscussionGroup(String groupId) {
		String sql = "select groupid from "
				+ DatabaseHelper.TABLES_NAME_GROUPS_2
				+ " where isdiscussion=1 and groupid ='" + groupId + "'";
		try {
			Cursor cursor = getInstance().sqliteDB().rawQuery(sql, null);
			if (cursor != null && cursor.getCount() > 0) {
				return true;
	...
	public static long updateGroup(ECGroup group) {
		...
		try {
			...
			return getInstance().sqliteDB().update(
					DatabaseHelper.TABLES_NAME_GROUPS_2, values, "groupid = ?",
	...
	public static int delGroup(String groupId) {
		try {
			...
			return getInstance().sqliteDB().delete(
					DatabaseHelper.TABLES_NAME_GROUPS_2, "groupid = ?",
					new String[] { groupId });
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public static int delALLGroup() {
		try {
			return getInstance().sqliteDB().delete(
					DatabaseHelper.TABLES_NAME_GROUPS_2, "isdiscussion=?",
					new String[] { "0" });
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public static List<String> getAllGroupIdBy(boolean joined) {
		...
		try {
			String sql = "select groupid from "
					+ DatabaseHelper.TABLES_NAME_GROUPS_2
					+ " where isdiscussion =0 and joined = " + (joined ? 1 : 0);
			cursor = getInstance().sqliteDB().rawQuery(sql, null);
			if (cursor.getCount() > 0) {
				while (cursor.moveToNext()) {
					String groupid = cursor.getString(0);
	...
	public static List<String> getAllDisGroupIdBy(boolean joined) {
		...
		try {
			String sql = "select groupid from "
					+ DatabaseHelper.TABLES_NAME_GROUPS_2
					+ " where isdiscussion =1 and joined = " + (joined ? 1 : 0);
			cursor = getInstance().sqliteDB().rawQuery(sql, null);
			if (cursor.getCount() > 0) {
				while (cursor.moveToNext()) {
					String groupid = cursor.getString(0);
	...
	public static ECGroup getECGroup(String groupId) {
		try {
			String sql = "select name, type, count ,permission ,joined ,declared ,owner , isnotice from "
					+ DatabaseHelper.TABLES_NAME_GROUPS_2
			...
			if (cursor != null && cursor.getCount() > 0) {
				...
				group.setPermission(ECGroup.Permission.values()[cursor
						.getInt(3)]);
	...
	public static boolean isGroupNotify(String groupId) {
		String sql = "select isnotice from "
				+ DatabaseHelper.TABLES_NAME_GROUPS_2 + " where groupid='"
				+ groupId + "'";
		Cursor cursor = getInstance().sqliteDB().rawQuery(sql, null);
	...
	public static boolean isExistDiscussionGroup() {
		String sql = "select groupid from "
				+ DatabaseHelper.TABLES_NAME_GROUPS_2 + " where isdiscussion='"
				+ 1 + "'";
		Cursor cursor = getInstance().sqliteDB().rawQuery(sql, null);
		if (cursor != null && cursor.getCount() > 0) {
