...
import com.ucan.app.common.helper.EmojiconHelper;
import com.ucan.app.ui.emoji.Emojicon;
import com.ucan.app.ui.emoji.People;
import com.ucan.app.common.model.Emoji;
...
public class EmoticonUtil {
	...
	public static void initEmoji() {
		 ...
		 * .getStringArray(R.array.emoji_code_file); if (stringArray != null) {
		...
		for (Emojicon emojicon : data) {
			if (emojicon.getEmoji() != null) {
				...
				if (unicode > 0xff) {
					emoji.setId(EmojiconHelper.getEmojiResource(UCApplication
							.getInstance().getApplicationContext(), unicode));
					emojis.add(emoji);
				}
			}
		}
	...
	private static ArrayList<Emoji> emojis = new ArrayList<Emoji>();
	...
	public ArrayList<Emoji> getEmojiCache() {
	 ...
	 *
	 */
	public static SpannableString getTextFormat(Context context, String format,
			int paddingSize) {
		...
		switch (paddingSize) {
		case -1:
			paddingSize = context.getResources().getDimensionPixelSize(
					R.dimen.ccp_button_text_size);
			break;
		case -2:
			paddingSize = context.getResources().getDimensionPixelSize(
		...
		boolean containsKeyEmoji = containsKeyEmoji(context, object,
				paddingSize);
		if (containsKeyEmoji) {
			hashMap.put(key, object);
		}
