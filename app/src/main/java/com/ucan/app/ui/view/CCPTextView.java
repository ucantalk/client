...
import com.ucan.app.common.utils.EmoticonUtil;
...
public class CCPTextView extends TextView {
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		if (mPreviewListener != null && mDetector != null) {
			...
			LogUtil.d(LogUtil.getLogUtilsTag(getClass()),
					"dispatcher onTouchEvent result " + result);
		}

		if (action == MotionEvent.ACTION_UP && mIgnoreNextActionUp) {
			LogUtil.d(LogUtil.getLogUtilsTag(getClass()),
					"ignore Action Up Event this time");
			if (!result) {
	...
	public boolean performLongClick() {
		LogUtil.d(LogUtil.getLogUtilsTag(getClass()),
				"performLongClick , should ignore Action Up Event next time");
		mIgnoreNextActionUp = true;
		return super.performLongClick();
	}

	public void setEmojiText(String text) {
		setText(EmoticonUtil.emoji2CharSequence(getContext(), text,
				(int) getTextSize(), false));
	}

	public void setEmojiText(CharSequence text) {
		setText(EmoticonUtil
				.emoji2CharSequence(getContext(),
